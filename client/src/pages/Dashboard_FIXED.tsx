...
function ConversationView({ conversationId }: { conversationId: number }) {
    ...
    const resumeAgent = useResumeAgent();  // NEW: Backend agent trigger
    ...
    const handleToggleAgent = () => {
        ...
        // FIX #4: Use resume endpoint when unpausing
        const isPaused = !conversation.isAgentActive || uiState?.agent_status === "PAUSED";

        if (isPaused) {
            // Resume via backend trigger
            resumeAgent.mutate(conversationId);
        } else {
            // Pause by deactivating
            ...
            <div className="lg:col-span-7 flex flex-col gap-6 h-full overflow-hidden">
                ...
                <div className="glass-panel p-4 rounded-xl flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        <ScamScore score={uiState?.risk_score ? Math.round(uiState.risk_score * 100) : (conversation?.scamScore || 0)} className="w-24 h-24 -my-2" />
                        <div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="font-bold tracking-tight">{uiState?.agent_status || conversation?.status?.toUpperCase()}</span>
                            </div>
                        </div>
                        <div className="h-8 w-px bg-white/10" />
                        <div>
                            <div className="text-xs font-mono text-muted-foreground uppercase mb-1">Intel Count</div>
                            <div className="font-bold text-xl">{uiState?.intel_count || reports?.length || 0}</div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        <CyberButton
                            size="sm"
                            variant={conversation?.isAgentActive && uiState?.agent_status !== "PAUSED" ? "danger" : "primary"}
                            onClick={handleToggleAgent}
                            isLoading={updateConversation.isPending || resumeAgent.isPending}
                        >
                            {!conversation?.isAgentActive || uiState?.agent_status === "PAUSED" ? "RESUME AI" : "PAUSE AI"}
                        </CyberButton>
            ...
            <div className="lg:col-span-5 flex flex-col gap-6 h-full overflow-hidden">
                ...
                <div className="h-[45%]">
                    <AgentBrain
                        ...
                        isActive={uiState?.agent_status === "ACTIVE"}
                    />
                </div>
